...
internal class MediaFireFileSystem(
    private val provider: MediaFireFileSystemProvider,
    val authority: Authority
) : FileSystem(), ByteStringListPathCreator, Parcelable {
    val rootDirectory = MediaFirePath(this, SEPARATOR_BYTE_STRING)
    ...
    val defaultDirectory: MediaFirePath
        get() = rootDirectory
    ...
    override fun supportedFileAttributeViews(): Set<String> =
        MediaFireFileAttributeView.SUPPORTED_NAMES

    override fun getPath(first: String, vararg more: String): MediaFirePath {
        val path = ByteStringBuilder(first.toByteString())
            .apply { more.forEach { append(SEPARATOR).append(it.toByteString()) } }
            .toByteString()
        return MediaFirePath(this, path)
    }

    override fun getPath(first: ByteString, vararg more: ByteString): MediaFirePath {
    ...
    companion object {
        ...
        @JvmField
        val CREATOR = object : Parcelable.Creator<MediaFireFileSystem> {
            override fun createFromParcel(source: Parcel): MediaFireFileSystem {
                val authority = source.readParcelable<Authority>()!!
                return MediaFireFileSystemProvider.getOrNewFileSystem(authority)
            }

            override fun newArray(size: Int): Array<MediaFireFileSystem?> = arrayOfNulls(size)
        }
    }
}
